...
describe('crud testing', () => {
  ...
  it('should return the updated object - generic query', async () => {
    ...
    return cloudFunctions.genericQuery(queryParams).then((result) => {
      ...
      const { AssessmentandEvaluationSurgical } = jsonValues[0];
      const { chronic_condition_hypertension } = jsonValues[0];
      const { yearsLivedinthecommunity } = jsonValues[0];
      const { latitude } = jsonValues[0];
      const { longitude } = jsonValues[0];
      const { fname } = jsonValues[0];
      const { lname } = jsonValues[0];
